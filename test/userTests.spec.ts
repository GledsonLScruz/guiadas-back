...
describe('Testes de repositório', function () {
    ...
    it('creates a new user', async function () {
        ...
        expect(u1.dataValues.username).to.equal(username);
        expect(u1.dataValues.email).to.equal(email);
        expect(u1.dataValues.password).to.equal(password);
        expect(u1.dataValues.startSemester).to.equal(semester);
        expect(Number(u1.dataValues.enrolledCourseId)).to.equal(courseId);
    ...
    it('updates an existing user', async function () {
        ...
        expect(await repo.updateUser(u1.dataValues.id, username, email, "password", semester, courseId)).to.equal(1);
    ...
    it('tries to delete an user using an invalid id', async function () {
        ...
        expect(deleteUser(u1.dataValues.id + 1)).to.equal(0);
...
describe('Testes de service', function () {
    ...
    it('creates a valid user', async function () {
        ...
        expect(u1.dataValues.password).to.equal(password);
        expect(u1.dataValues.startSemester).to.equal(semester);
        expect(u1.dataValues.enrolledCourseId).to.equal(String(courseId));
    ...
    it('successfully updates a user', async function () {
        ...
        updateUser(u1.dataValues.id, username, email, "novasenha", semester, String(courseId));

        const users = await getUsers();
        const user = users[0];
        expect(user.dataValues.password).to.equal("novasenha")
    ...
    it('tries to update a user using a wrong id', async function () {
        ...
        updateUser(u1.dataValues.id + 1, username, email, "novasenha", semester, String(courseId));

        const users = await getUsers();
        expect(users[0].dataValues.password).to.equal(password);
